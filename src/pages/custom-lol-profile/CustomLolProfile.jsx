...
import {
  getBasicInfo,
  getLast30Matches,
  getThreeMostPlayedChamps,
  getAllGameDetails,
  getCurrentSeasonInfo,
} from '../../riot-data-management/fetches/riot-fetches';
import {
  changeCustomProfileStatusInDB,
  createsProfileUserInDB,
} from './users-utils';
...
const GAME_STYLES = [
  'Looking for a team',
  'Looking for a duo',
  'Just for casual playing',
];

export default function CustomLolProfile() {
  let [, , isCustomProfileCreated, setIsCustomProfileCreated] =
  ...
  const getProfileData = async (event) => {
    ...
    const firstThreeChampsMostPlayed = await getThreeMostPlayedChamps(
    ...
    await createsProfileUserInDB({
      stats: { mean },
      infoAccount: { basicInfo },
      seasonInfo: { seasonInfo },
      champs: { firstThreeChampsMostPlayed },
      roles: { roles },
      ...
      >
        <Box sx={style}>
          <Box display="flex" flexDirection="column" gap={2}>
            <Typography id="modal-modal-title" variant="h5" color="secondary">
              Find players to play with!
            </Typography>
            <Typography variant="body1">
              Please fill the form to publish the offer in the billboard:
            </Typography>
          </Box>
              ...
              <Box display="flex" flexDirection="column" ml={5} gap={2}>
                <Box>
                  <Typography color="primary">
                    Please insert your summoner name
                  </Typography>
                </Box>
