  ...
  async function handleUploadedFile(e) {
    ...
    nsfwjs
      ...
      .then((predictions) => {
        ...
        const sensitiveFlags = predictions.filter(
          ({ className, probability }) =>
            probability > sensitiveParameters[className]
        );
        console.log(sensitiveFlags, sensitiveFlags.length);

        const sensitiveMessage = !!sensitiveFlags.length
          ? sensitiveFlags.map((flag) => {
              return `${
                flag.className
              } probability is ${flag.probability.toFixed(3)}`;
            })
